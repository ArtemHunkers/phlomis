...
nameInputs.forEach((input) => {
    input.addEventListener('keypress', function (e) {
        if (e.keyCode != 8 && e.keyCode != 46 && (input.value.length > 49 || e.key && e.key.match(/[^а-яА-ЯЁёІіЇїЄєa-zA-ZẞßÄäÜüÖöÀàÈèÉéÌìÍíÎîÒòÓóÙùÚúÂâÊêÔôÛûËëÏïŸÿÇçÑñŒ''`'.-\s]/)))
            return e.preventDefault();
    });
    input.addEventListener('input', function (e) {
        if (e.inputType == "insertFromPaste") {
            input.value = input.value.replace(/[^а-яА-ЯЁёІіЇїЄєa-zA-ZẞßÄäÜüÖöÀàÈèÉéÌìÍíÎîÒòÓóÙùÚúÂâÊêÔôÛûËëÏïŸÿÇçÑñŒ''`'.-\s]/g, "").slice(0, 50);
        }
    });
});
...
function formValidation(form) {
    ...
    inputs.forEach((input) => {
        ...
        if (input.hasAttribute('data-name-input')) {
            const namePattern = /[^а-яА-ЯЁёІіЇїЄєa-zA-ZẞßÄäÜüÖöÀàÈèÉéÌìÍíÎîÒòÓóÙùÚúÂâÊêÔôÛûËëÏïŸÿÇçÑñŒ''`'.-\s]/g;
...
$('form').on('submit', function (e) {
    ...
    if (formValidation(form[0])) {
        ...
        formData.set('routeKey', 'samolet_port_emm');
...
function sendRequest(url, form, formData) {
    ...
    callTouchFormData.set('routeKey', 'samolet_port_emm');
    ...
    var callTouchOptions = {
        ...
        body: new URLSearchParams(callTouchFormData).toString()
    }

    button.addClass('pointer-events-none');
    const sendToApi = fetch(url, options)
    const sendToCallTouch = new Promise((resolve, reject) => {
        window.ctw.createRequest('samolet_port_emm', formData.get('phone'),
        [
            {
                "name": "Имя",
    ...
    Promise.all([
        ...
        if (sendToApiResult.status == 200 && sendToCallTouchResult) {
            ...
            console.log('Error in API or CallTouch request');
        }
    }).catch((error) => {
        button.removeClass('pointer-events-none');
        showErrorMessage(form, 'Произошла ошибка при отправке формы. Попробуйте позже или свяжитесь с нами другим способом.')
